...
async function main() {
  ...
  const applier: HotpApplier = { async applyInTx(_e: PgExecutor, _r) { /* checkpoint+applied row are the durable proof */ } };
  ...
  const nonceStore = new PgReplayNonceStore((sql, params) => b.pool.query(sql, params as never) as never);
  const ingest = createHttpOutboxReceiver({ resolveRequestKey: (kid) => (kid === REQ_KEY ? reqSecret : null), receive, nonceStore });
  ...
  const transport = new HttpOutboxTransport({ url: transportUrl, fetch: fetch as never, requestKeyId: REQ_KEY, requestSecret: reqSecret, ackVerifier, timeoutMs: 3_000 });
  ...
  try {
    await check('(1) happy A->B: append N -> B applies strictly 1..N over the authenticated transport, HOTP once', async () => {
      const N = 8;
      const t0 = Date.now();
      await appendN(1, N);
      await drain();
    ...
    await check('(2) network partition drops the ACK after B applied -> retriable, no double-apply; heal converges (RPO/RTO)', async () => {
      ...
      const rpo = await unackedA(poolA); // rows durably on A not yet applied+ACKED on B (redelivered as duplicate-ok)
      ...
      assert.ok(rpo >= 1, `unconverged tail present (RPO=${rpo} rows durable-on-A-not-yet-acked)`);
      ...
      console.log(`     RPO=${rpo} rows (durable, redelivered duplicate-ok) RTO=${rtoMs}ms to converge`);
    });

    await check('(6) node-B PostgreSQL unavailable -> publisher retriable; recovery converges (RTO)', async () => {
      const base = await rcvSeqB(probeB);
