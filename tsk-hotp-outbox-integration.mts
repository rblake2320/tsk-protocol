...
const mkOutbox = (db: PgTransactor, sid: string, ready = READY) => new UnfencedSingleNodeTskDurableOutbox(db, ready, { streamId: sid, sanitizer, signer, maxPendingRows: 100_000, backpressure: 'fail-authoritative-mutation' });
...
async function main() {
  ...
  await check('attestation catches drift; readiness token is unforgeable + transactor-bound', async () => {
    ...
    assert.throws(() => new UnfencedSingleNodeTskDurableOutbox(other, READY, { streamId: 's/v1', sanitizer, signer, maxPendingRows: 1, backpressure: 'quarantine' }), /different PgTransactor/);
    assert.throws(() => new UnfencedSingleNodeTskDurableOutbox(serial, {} as SchemaReadyToken, { streamId: 's/v1', sanitizer, signer, maxPendingRows: 1, backpressure: 'quarantine' }), /forged or foreign/);
  });
  ...
  await check('(TOCTOU) append: mutating the raw mutation during signer.sign does not change stored/digested/signed bytes', async () => {
    ...
    const ob = new UnfencedSingleNodeTskDurableOutbox(serial, READY, { streamId: sid, sanitizer, signer: mutatingSigner, maxPendingRows: 100, backpressure: 'quarantine' });
